...
//			DriverManager.registerDriver(new Driver());// 会导致驱动注册两次。
			...
			// 2.获得连接
			conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/jdbctest?useSSL=false&serverTimezone=Hongkong&useUnicode=true&characterEncoding=utf-8", "root", "123456");
			...
			while(rs.next()){
				...
				System.out.println("demo2:"+uid+"   "+username+"   "+password+"   "+name);
			}
...
//			DriverManager.registerDriver(new Driver());// 会导致驱动注册两次。
			...
			while(resultSet.next()){
				...
				System.out.println("demo1:"+uid+"   "+username+"   "+password+"   "+name);
			}
			// 4.释放资源
			resultSet.close();
			stmt.close();
